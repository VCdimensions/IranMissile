...
uae_daily = [{'date': '2026-03-01', 'missiles': 28, 'drones': 332, 'missiles_pct': None, 'drones_pct': None, 'note': '由 3/1 累計（165/541）減去 2/28 起始累計（137/209）反推；亦與官方文內『20枚攔截＋8枚落海；311架擊落＋21架擊中/落地』一致。'}, {'date': '2026-03-02', 'missiles': 9, 'drones': 148, 'missiles_pct': -67.85714285714286, 'drones_pct': -55.42168674698795, 'note': '官方逐日公告。另有 6 枚巡弋飛彈遭攔截，但本頁主統計僅納入飛彈/無人機兩類。'}, {'date': '2026-03-03', 'missiles': 12, 'drones': 123, 'missiles_pct': 33.33333333333333, 'drones_pct': -16.89189189189189, 'note': '官方逐日公告。'}, {'date': '2026-03-04', 'missiles': 3, 'drones': 129, 'missiles_pct': -75.0, 'drones_pct': 4.878048780487805, 'note': '官方逐日公告。'}, {'date': '2026-03-05', 'missiles': 7, 'drones': 131, 'missiles_pct': 133.33333333333331, 'drones_pct': 1.550387596899225, 'note': '官方逐日公告。'}, {'date': '2026-03-06', 'missiles': 9, 'drones': 112, 'missiles_pct': 28.57142857142857, 'drones_pct': -14.50381679389313, 'note': '官方逐日公告。'}, {'date': '2026-03-07', 'missiles': 16, 'drones': 121, 'missiles_pct': 77.77777777777779, 'drones_pct': 8.035714285714286, 'note': '官方逐日公告。'}, {'date': '2026-03-08', 'missiles': 17, 'drones': 117, 'missiles_pct': 6.25, 'drones_pct': -3.3057851239669422, 'note': '官方逐日公告。'}]
norm_series = [1.0, 0.4361, 0.375, 0.3667, 0.3833, 0.3361, 0.3806, 0.3722]
decay_series = [1.0, 0.8683, 0.754, 0.6547, 0.5685, 0.4936, 0.4286, 0.3722]
...
m1.metric("UAE March-to-date 飛彈（3/1–3/8）", fmt_int(sum_missiles), help="逐日官方統計合計")
m2.metric("UAE March-to-date 無人機（3/1–3/8）", fmt_int(sum_drones), help="逐日官方統計合計")
...
with tab1:
    ...
    with left:
        st.subheader("UAE 逐日圖表（2026/03/01–03/08）")
        st.caption("雙 Y 軸 bar chart：左軸 = 飛彈、右軸 = 無人機；另加上 normalized 日總量折線與指數衰減參考線。")
        fig = go.Figure()
        x_full = [d['date'] for d in uae_daily]
        x_labels = [pd.to_datetime(d['date']).strftime('%m/%d') for d in uae_daily]
        hover_dates = [pd.to_datetime(d['date']).strftime('%Y/%m/%d') for d in uae_daily]
        fig.add_bar(
            x=x_labels,
            y=[d['missiles'] for d in uae_daily],
            name='飛彈（左軸）',
            marker_color=BLUE,
            yaxis='y',
        ...
        fig.add_trace(
            go.Scatter(
                ...
                yaxis='y3',
                customdata=hover_dates,
                hovertemplate='日期：%{customdata}<br>Normalized：%{y:.3f}<extra></extra>',
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                yaxis='y3',
                customdata=hover_dates,
                hovertemplate='日期：%{customdata}<br>參考線：%{y:.3f}<extra></extra>',
            )
        )
        fig.update_layout(
            barmode='group',
            ...
            yaxis3=dict(title='Normalized', overlaying='y', side='right', position=0.94, showgrid=False, range=[0, 1.08]),
            height=520,
        )
    ...
    with right:
        ...
        <div class="note-box">
          <h3>圖表解讀</h3>
          <ul>
            <li><strong>3/1 為高峰：</strong> 當日反推 28 枚飛彈、332 架無人機，為目前 March-to-date 峰值。</li>
            <li><strong>之後快速鈍化：</strong> UAE 日總量從 3/1 的 360，降至 3/8 的 134；normalized 由 1.00 降至 0.37。</li>
        ...
        <div class="note-box" style="margin-top:14px;">
          ...
          <ul>
            ...
            <li>3/1 數字不是單獨日報，而是以 3/1 與 2/28 兩個官方累計點位反推得到。</li>
            <li>normalized = 當日（飛彈 + 無人機）÷ 3/1 當日（飛彈 + 無人機）。</li>
            <li>指數衰減參考線以 3/1 與 3/8 兩端點校準。</li>
          </ul>
        </div>
        """, unsafe_allow_html=True)
